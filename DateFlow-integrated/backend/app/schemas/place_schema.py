...
class PlaceBookingEnrichResponse(BaseModel):
    total: int
    enriched: int
    skipped: int


class RegionCollectRequest(BaseModel):
    region: str = Field(..., description="지역명 (예: '홍대', '강남', '용인')")
    latitude: float = Field(..., description="중심 위도")
    longitude: float = Field(..., description="중심 경도")
    radius_m: int = Field(1000, ge=100, le=20000, description="검색 반경 (미터)")


class TrackCountResponse(BaseModel):
    category: int
    keyword: int
    blog: int


class RegionCollectResponse(BaseModel):
    region: str
    total: int
    by_track: TrackCountResponse
    places: list[PlaceResult]
